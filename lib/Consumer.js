...
class Consumer extends EnhancedEventEmitter
{
	constructor(internal, data, channel)
	{
		...
		logger.debug('constructor() data=%s', JSON.stringify(data));
	...
	enable(transport, remotelyPaused, preferredProfile)
	{
		...
		return this._channel.request(
			'consumer.enable', internal, { rtpParameters: this.rtpParameters })
			.then(() =>
			{
	...
	setPreferredProfile(profile)
	{
		...
		this._channel.request(
			'consumer.setPreferredProfile', this._internal, { profile })
			.then(() =>
			{
