	...
	createRtpStreamer(producer, options)
	{
		...
		return Promise.resolve()
			...
			.then(() =>
			{
				...
				// remove from each rtpParameters.codecs ffmpeg-proxy mentioning - unless sendOldNack is true
				if (!options.sendOldNack)
				{
					consumerRtpParameters.forEach((codec) => {
						codec.rtcpFeedback = codec.rtcpFeedback.filter(function(value, index, arr) {
								return value.type !== 'ffmpeg-proxy';
							});
					});
				}
