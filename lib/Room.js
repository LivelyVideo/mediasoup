	...
	createRtpStreamer(producer, options)
	{
		...
		return Promise.resolve()
			...
			.then(() =>
			{
				...
				// Remove ffmpeg-proxy from capabilities unless sendOldNack is set
				if (options.sendOldNack !== true)
				{
					consumerRtpParameters.codecs.forEach((codec) => {
						...
						{
							let dbgfeed = codec.rtcpFeedback.filter((val) => val.type === 'ffmpeg-proxy'); // TODO: remove before checkin
							if (dbgfeed.length > 0)
								logger.debug('"createRtpStreamer()" has ffmpeg-proxy rtcp-fb: %s', dbgfeed);

							let rtcpFeedback = codec.rtcpFeedback.filter((val) => val.type !== 'ffmpeg-proxy');
							if (rtcpFeedback.length > 0)
								logger.debug('"createRtpStreamer()" with removed ffmpeg-proxy rtcp-fb: %s', rtcpFeedback);
				...
				{
					logger.debug('"createRtpStreamer()" no changes in ffmpeg-proxy rtcp-fb '); // TODO: remove before checkin
				}
