...
const getCpuUsage = function(pid, cb) 
{
...
{
	constructor(id, parameters, options = {})
	{
		super();
		...
		*/
		...
		this._cpuPercentage = null;
		if (options.monitorCpu) {
			this._cpuInterval = setInterval(() => {
				if (this._child) {
					getCpuUsage(this._child.pid, (startTime) => {
						setTimeout(() => {
							if (this._child) {
								getCpuUsage(this._child.pid, (endTime) => {
									const delta = endTime - startTime;
									const percentage = 100 * (delta / 10000);
									this._cpuPercentage = percentage;
								});	
							}
						}, 1000);
					});	
