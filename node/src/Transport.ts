...
function createConsumeRequest({
	...
	// Serialize Lively appData fields
	let callIdOffset: number | undefined;
	let peerIdOffset: number | undefined;
	let mirrorIdOffset: number | undefined;
	let streamNameOffset: number | undefined;

	if (appData) {
		if (appData.callId) {
			callIdOffset = builder.createString(appData.callId);
		}
		if (appData.peerId) {
			peerIdOffset = builder.createString(appData.peerId);
		}
		if (appData.mirrorId) {
			mirrorIdOffset = builder.createString(appData.mirrorId);
		}
		if (appData.streamName) {
			streamNameOffset = builder.createString(appData.streamName);
		}
	}
	...
	if (callIdOffset !== undefined) {
		ConsumeRequest.addCallId(builder, callIdOffset);
	}
	if (peerIdOffset !== undefined) {
		ConsumeRequest.addPeerId(builder, peerIdOffset);
	}
	if (mirrorIdOffset !== undefined) {
		ConsumeRequest.addMirrorId(builder, mirrorIdOffset);
	}
	if (streamNameOffset !== undefined) {
		ConsumeRequest.addStreamName(builder, streamNameOffset);
	}
